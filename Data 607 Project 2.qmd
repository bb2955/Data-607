...
```{r}
ggplot(tidy_rundisney, aes(age_group, avg_finish_time_minutes, fill = gender)) +
...
```{r}
ggplot(tidy_disney, aes(month, avg_wait_minutes, color = attraction, group = attraction)) +
  geom_line() +
  facet_wrap(~park) +
  labs(
...
```{r}
ggplot(tidy_amc, aes(month, tickets_sold, fill = movie_genre)) +
  geom_col(position = "dodge") +
  facet_wrap(~location) +
  labs(
